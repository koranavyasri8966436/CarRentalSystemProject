...
public class UpdateCar extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    car = new Car(carNo,carModel,carColor,status,capacity,basePrice,costPerKm,penality,image,rating,location);	
		carDao = new CarDao();
		int x = carDao.update(car);
		if(x > 0){
			List<Car>arrayList = carDao.getAllCars();
			request.setAttribute("carsData", arrayList);
			out.println("<h3><CENTER>Update Success .."+x+" Cars Updated...</CENTER></H3>");
